...
class ProvisionMinecraftServerStateMachine(Construct):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        deploy_server = sfn.Choice(
            self,
            id=self.namer("Deploy or Destroy"),
        ).when(condition=sfn.Condition.string_equals("$.command", "deploy"), next=submit_cdk_deploy_batch_job)

        self.state_machine = sfn.StateMachine(
            scope=self,
            id=self.namer("StateMachine"),
            definition=validate_execution_input.next(deploy_server),
            # logs=sfn.LogOptions(),
            role=None,
        )
